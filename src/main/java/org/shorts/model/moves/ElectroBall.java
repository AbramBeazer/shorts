...
public class ElectroBall extends Move implements BallBombMove {
    ...
    public ElectroBall() {
        super("Electro Ball", 40, 100, Type.ELECTRIC, Category.SPECIAL, Range.SINGLE_ADJACENT_ANY, 16, false, 0);
    }

    @Override
    protected double getPowerMultipliers(Pokemon user, Pokemon target, Battle battle) {
        double percentage = target.calculateSpeed() / (double) user.calculateSpeed();
